...
export function SuggestedQuestions({ onSelect, disabled }) {
  return (
    ...
    >
      ...
      <div className="flex overflow-x-auto gap-2 pb-2 pt-0.5 -mx-1 px-1 scroll-smooth snap-x">
        {QUESTIONS.map((q, i) => (
          <motion.button
            key={q.text}
            ...
            disabled={disabled}
            className="question-card flex-shrink-0 snap-start rounded-xl px-3 py-2 text-left cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed whitespace-nowrap"
          >
